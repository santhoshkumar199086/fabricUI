...
const StatusBadge = ({ status, online = true }) => (
  <div className="flex items-center space-x-2">
    ...
    <span className={`text-sm font-medium ${online ? 'text-green-700' : 'text-red-700'}`}>
      {status}
    </span>
  </div>
);
...
const MetricCard = ({ title, value, unit, icon, color = "blue", subtitle, trend }) => {
  const colorMap = {
    blue: { bg: "from-blue-50 to-blue-100", border: "border-blue-200", text: "text-blue-600", accent: "bg-blue-500" },
    green: { bg: "from-green-50 to-green-100", border: "border-green-200", text: "text-green-600", accent: "bg-green-500" },
    orange: { bg: "from-orange-50 to-orange-100", border: "border-orange-200", text: "text-orange-600", accent: "bg-orange-500" },
    purple: { bg: "from-purple-50 to-purple-100", border: "border-purple-200", text: "text-purple-600", accent: "bg-purple-500" },
    red: { bg: "from-red-50 to-red-100", border: "border-red-200", text: "text-red-600", accent: "bg-red-500" }
  };

  const colors = colorMap[color];

  return (
    <div className={`bg-gradient-to-br ${colors.bg} ${colors.border} border-2 rounded-xl p-6 transition-all duration-300 hover:shadow-xl hover:scale-105 transform`}>
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center space-x-3 mb-3">
            <div className={`${colors.accent} p-2 rounded-lg text-white text-lg`}>
              {icon}
            </div>
            <div>
              <p className={`text-sm font-bold ${colors.text} uppercase tracking-wide`}>{title}</p>
...
const EnhancedTelemetryDashboard = () => {
  ...
  const { data: influxData, isLoading } = useInfluxData(timeRange, measurementTypes);

  // Calculate enhanced statistics
  const stats = useMemo(() => {
    const cpuData = influxData.filter(item => item._measurement === "dell_cpu");
    const memData = influxData.filter(item => item._measurement === "dell_mem");
    const arpData = influxData.filter(item => item._measurement === "dell_arp");
    const interfaceData = influxData.filter(item => item._measurement === "dell_interface_status");

    const latestCpu = cpuData.length > 0 ? (cpuData[cpuData.length - 1]._value / 100).toFixed(1) : 0;
    const latestMem = memData.length > 0 ? (memData[memData.length - 1]._value / 1024).toFixed(1) : 0;
    const activeInterfaces = interfaceData.filter(item => item._value === "up").length;
    ...
    return {
      ...
      activeInterfaces,
      totalInterfaces,
      interfaceHealth: totalInterfaces > 0 ? ((activeInterfaces / totalInterfaces) * 100).toFixed(0) : 0
    };
  }, [influxData]);
  ...
  const dellCpuData = influxData.filter(item => item._measurement === "dell_cpu");
  const dellMemData = influxData.filter(item => item._measurement === "dell_mem");
  ...
  const dellArpData = influxData
    ...
    .map(item => ({
      ...
      _value: item._field === "age-time" ? formatMicroseconds(item._value) : item._value
    }));
  ...
  const dellInterface = influxData
    ...
    .map(item => ({
      ...item,
      _time: formatTime(item._time),
      _value: item._field === "_time" ? formatMicroseconds(item._value) : item._value
    }));

  const dellMacData = influxData.filter(item => item._measurement === "dell_mac_table");
  ...
  const dellRouting = influxData
    ...
    .map(item => ({
      ...item,
      _time: formatTime(item._time),
      _value: item._field === "last-updated" ? formatMicroseconds(item._value) : item._value
    }));
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 mb-8 overflow-hidden">
          ...
          <div className="p-6">
            ...
            <div className="mb-8">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">CPU Utilization Trends</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div>
                <h3 className="text-lg font-semibold text-gray-800 mb-4">CPU Metrics</h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-8 shadow-2xl">
            <div className="flex items-center space-x-4">
              ...
              <p className="text-lg font-semibold text-gray-700">Loading telemetry data...</p>
            </div>
          </div>
        </div>
      )}
