...
import waveChartData from '../../Helpers/waveChart.json'

// Enhanced CPU Wave Chart
// { data = [] }
const DellCpuWaveChart = ({ data = [] }) => {
  //const data = waveChartData;
    ...
    return [
      {
        ...
        data:
          groupByField["cpu-util-1min"]
            ?.map((item) => ({
              x: new Date(item._time).getTime(),
              y: Number((item._value / 100).toFixed(2)),
            }))
            .sort((a, b) => a.x - b.x) || [],
      },
      {
        ...
        data:
          groupByField["mem-threshold-hi"]?.map((item) => ({
            x: new Date(item._time).getTime(),
            y: item._value,
          })) || [],
      },
      {
        name: "Memory Low Threshold",
        data:
          groupByField["mem-threshold-lo"]?.map((item) => ({
            x: new Date(item._time).getTime(),
            y: item._value,
          })) || [],
      },
  ...
  const options = {
    ...
    colors: ["#3B82F6", "#EF4444", "#F59E0B"], // Added colors for each series
    dataLabels: {
      enabled: false,
    },
    xaxis: {
...
//       .filter((_field) => allowedFields.includes(_field))
//       .map((_field) => ({
//         name: _field.replace("-", " ").toUpperCase(),
//         data: groupByField[_field]
//           .map((point) => ({
...
//   return <Chart options={options} series={chartData} type="bar" height={350} />;
// };
...
//     return Object.keys(groupByMemoryField).map((_field) => ({
//       name: _field.replace("-", " ").toUpperCase(),
//       data: groupByMemoryField[_field]
//         .map((point) => ({
//           x: new Date(point._time).getTime(),
